...
export function* dislikePostWorker ({ payload: postID }) {
    try {
        ...
        yield put(postActions.dislikePostSucceed({ postID, userID })); // authSUCCEED - true
        yield put(replace(pages.feed)); // jump into feed component after sign up
    } catch (error) {
        yield put(postActions.dislikePostFailed(error.message)); // dispatch all failed actions for sign up
    } finally {
